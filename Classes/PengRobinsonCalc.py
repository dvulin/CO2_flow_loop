...
class PengRobinsonCalc(EOSCalc):
    def __init__(self, components, T, P, z):
        ...
        self.A = (self.a_mix * self.P) / (self.R**2 * self.T**2)
        self.B = (self.b_mix * self.P) / (self.R * self.T)
    ...
    def solve_eos(self, use_z_form=False):
        if use_z_form:
            # rješavanje kubne jednadžbe za Z
            A, B = self.A, self.B
            coeffs = [1, -(1 - B), A - 3*B**2 - 2*B, -(A*B - B**2 - B**3)]
            roots = np.roots(coeffs)
            real_roots = roots[np.isreal(roots)].real
            Z = max(real_roots)  # pretpostavljamo plinsku fazu
            V = Z * self.R * self.T / self.P
            return V
        else:
